...
function getSimilarityTransformation(fromPoints,
    ...
    const dimensions_all = allPoints.rows;
    const numPoints_all = allPoints.columns;

    // 4. Transform the points.
    const transformedPoints = mlMatrix.Matrix.add(
        mlMatrix.Matrix.mul(rotation.mmul(allPoints), scale),
        translation.repeat({columns: numPoints_all}));
